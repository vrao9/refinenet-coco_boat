...
run_config.root_cache_dir=fullfile('D:/TUHH/Arbeit/refinenet-master/refinenet-master/cache_data', ds_name, model_name);
...
%{
%(vij)
ds_info = gen_ds_info_fn(ds_config);
%(vij)
ds_info.use_custom_data=true;

%%%%%%%%%%%%%%%%%%%

trying out!
names = [fieldnames(ds_info); fieldnames(ds_config)];
struct3 = cell2struct([struct2cell(ds_info); struct2cell(ds_config)], names, 1);
